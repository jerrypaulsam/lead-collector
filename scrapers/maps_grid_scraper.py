...
STATE_FILE = "maps_grid_state.json"  
...
# Added 'seen' parameter to cross-reference our global cache
async def scrape_cell(context, query, lat, lon, results, limit, seen):
	search_page = await context.new_page()

	try:
		links = await collect_links(search_page, query, lat, lon)
		print(f"    -> Collected {len(links)} raw links in cell.")

		processed_new = 0

		for link in links:
			if len(results) >= limit or processed_new >= PER_CELL_LIMIT:
				break
			...
			if data:
				results.append(data)
				processed_new += 1 # Only tick the cell limit up if we actually found new data
...
async def worker(context, queue, query, results, limit, seen):
	while not queue.empty():
		...
		await scrape_cell(context, query, lat, lon, results, limit, seen)
		queue.task_done()
...
async def run(query, limit, city):
	...
	async with async_playwright() as p:
		...
		for _ in range(GRID_WORKERS):
			tasks.append(
				asyncio.create_task(
					worker(context, queue, query, results, limit, seen)
				)
			)
